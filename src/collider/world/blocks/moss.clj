...
(defn carpet-updated
  "Returns the carpet st at p with its sides set.
  Each side comes from what stands beside it. When create-sides?
  is true, it also adds sides it does not have yet."
...
(defn carpet-topper
  "Returns the carpet state to place above p.
  Returns nil when none fits there. side? is a predicate on a
  direction and tells which sides it may keep."
  [chunks p side?]
...
(defn hanging-tip ^long [chunks p ^long st]
  (with st :tip (if (= (block/block-of st) (block/block-of (chunk/at-void chunks (dir/down p)))) :false :true)))
...
(defn hanging-end [chunks p self]
  (loop [q (dir/down p)]
    (if (= self (block/block-of (chunk/at-void chunks q))) (recur (dir/down q)) q)))
