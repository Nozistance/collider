(ns collider.world.blocks.dripstone
  "Pointed dripstone and sulfur spikes, their growth and drip."
  (:require [collider.world.block :as block]
            [collider.world.direction :as dir]
            [collider.world.chunk :as chunk]
            [collider.world.env.attribute :as attribute]))

(set! *warn-on-reflection* true)

(def ^:private grows-on {:pointed-dripstone :dripstone-block})

(def ^:private max-growth-length
  {:pointed-dripstone 7 :sulfur-spike 2})

(def ^:private cauldrons
  #{:cauldron :layered-cauldron :lava-cauldron})

(def ^:private water-chance 0.17578125)

(def ^:private lava-chance 0.05859375)

(def ^:private growth-chance 0.011377778)

(defn speleothem?
  "Returns true when st is a spike that grows from stone.
  Pointed dripstone is such a spike."
  [^long st] (block/tagged? st "speleothems"))

(defn- dir-of [^long st] (:vertical-direction (block/props-of st)))

(defn- thickness-of [^long st] (:thickness (block/props-of st)))

(defn- directed? [^long st dir]
  (and (speleothem? st) (= dir (dir-of st))))

(defn stalactite? [^long st] (directed? st :down))

(defn- stalagmite? [^long st] (directed? st :up))

(defn- tip? [^long st merged?]
  (let [th (thickness-of st)]
    (and (speleothem? st)
         (or (= :tip th) (and merged? (= :tip_merge th))))))

(defn- free-hanging? [^long st]
  (and (stalactite? st)
       (= :tip (thickness-of st))
       (= :false (:waterlogged (block/props-of st)))))

(defn- unmerged-tip? [^long st dir self]
  (and (tip? st false) (= dir (dir-of st))
       (= self (block/block-of st))))

(defn- fluid-of [^long st]
  (cond
    (not (pos? st)) nil
    (block/waterlogged? st) :water
    :else (block/liquid-class st)))

(defn- water-at? [chunks p]
  (= :water (fluid-of (chunk/at-void chunks p))))

(defn- water-source-at? [chunks p]
  (let [st (chunk/at-void chunks p)]
    (and (= :water (fluid-of st))
         (or (block/waterlogged? st) (block/source-state? st)))))

(defn- toward [p dir] (mapv + p (dir/offset dir)))

(defn valid-placement? [chunks p dir self]
  (let [b (chunk/at-void chunks (toward p (dir/opposite dir)))]
    (and (not (neg? b))
         (or (block/face-sturdy? b dir)
             (and (directed? b dir) (= self (block/block-of b)))))))

(defn supported? [chunks p ^long st]
  (valid-placement? chunks p (dir-of st) (block/block-of st)))

(defn- facing-tip [front merge?]
  (if (and (not merge?) (not= :tip_merge (thickness-of front)))
    :tip
    :tip_merge))

(defn- thickness [chunks p dir merge? self]
  (let [base (dir/opposite dir)
        front (chunk/at-void chunks (toward p dir))
        back (chunk/at-void chunks (toward p base))]
    (cond
      (and (directed? front base) (= self (block/block-of front)))
      (facing-tip front merge?)
      (not (directed? front dir)) :tip
      (contains? #{:tip :tip_merge} (thickness-of front)) :frustum
      (directed? back dir) :middle
      :else :base)))

(defn updated ^long [chunks p ^long st]
  (let [dir (dir-of st) self (block/block-of st)
        merge? (= :tip_merge (thickness-of st))]
    (if-not (valid-placement? chunks p dir self)
      st
      (let [th (thickness chunks p dir merge? self)
            props (assoc (block/props-of st) :thickness th)]
        (block/state self props)))))

(defn- placed-dir [chunks p pitch self]
  (let [default (if (neg? (double pitch)) :down :up)
        other (dir/opposite default)]
    (cond
      (valid-placement? chunks p default self) default
      (valid-placement? chunks p other self) other)))

(defn placed [chunks p st pitch sneaking?]
  (let [st (long st)
        self (block/block-of st)]
    (when-let [dir (placed-dir chunks p pitch self)]
      (let [th (thickness chunks p dir (not sneaking?) self)
            props (assoc (block/props-of st)
                    :vertical-direction dir :thickness th)]
        (block/state self props)))))

(defn- find-vertical [chunks p dir path? target? max-steps]
  (loop [i 1 q (toward p dir)]
    (when (< i (long max-steps))
      (let [st (chunk/at-void chunks q)]
        (cond
          (neg? st) nil
          (target? st) q
          (path? st) (recur (inc i) (toward q dir))
          :else nil)))))

(defn- along [self dir]
  (fn [n] (and (= self (block/block-of n)) (= dir (dir-of n)))))

(defn- find-tip [chunks p ^long st ^long max-len]
  (if (tip? st false)
    p
    (let [dir (dir-of st) self (block/block-of st)]
      (find-vertical chunks p dir (along self dir) #(tip? % false)
                     max-len))))

(defn- root-pos [chunks p ^long st]
  (let [self (block/block-of st) dir (dir-of st)]
    (find-vertical chunks p :up (along self dir)
                   #(not= self (block/block-of %)) 11)))

(defn- fluid-over [dim ^long n]
  (if (and (= :mud (block/block-of n))
           (not (attribute/water-evaporates? dim)))
    :water
    (fluid-of n)))

(defn- fluid-above [chunks dim p ^long st]
  (when (stalactite? st)
    (when-let [root (root-pos chunks p st)]
      (let [q (dir/up root) n (chunk/at-void chunks q)]
        (when-not (neg? n)
          {:pos q :state n :fluid (fluid-over dim n)})))))

(defn- box-in-column? [[x0 y0 z0 x1 y1 z1]]
  (and (> (double x1) 6.0) (< (double x0) 10.0)
       (> (double y1) 0.0) (< (double y0) 16.0)
       (> (double z1) 6.0) (< (double z0) 10.0)))

(defn- drips-through? [^long st]
  (cond
    (zero? st) true
    (neg? st) false
    (block/solid-render? st) false
    (some? (fluid-of st)) false
    :else (not (some box-in-column? (block/collision-boxes st)))))

(defn- receives? [^long st fluid]
  (case (block/type-of st)
    :cauldron true
    :layered-cauldron
    (and (= :water fluid) (= :water-cauldron (block/block-of st)))
    false))

(defn- raised [^long st]
  (let [lvl (block/prop-long st :level)]
    (when (< lvl 3)
      (block/state (block/block-of st)
                   {:level (keyword (str (inc lvl)))}))))

(defn- filled [^long st fluid]
  (case (block/type-of st)
    :cauldron (if (= :water fluid)
                (block/state :water-cauldron {:level :1})
                (block/state :lava-cauldron))
    :layered-cauldron (raised st)
    nil))

(defn- start-pos? [chunks p ^long st]
  (let [above (chunk/at-void chunks (dir/up p))]
    (and (stalactite? st)
         (not= (block/block-of st) (block/block-of above)))))

(defn- fillable-cauldron [chunks tip fluid]
  (find-vertical chunks tip :down drips-through?
                 #(receives? % fluid) 11))

(defn- tip-above-cauldron [chunks p]
  (find-vertical chunks p :up drips-through? free-hanging? 11))

(defn- fill-fluid [chunks dim tip]
  (let [st (chunk/at-void chunks tip)
        f (:fluid (fluid-above chunks dim tip st))]
    (when (contains? #{:water :lava} f) f)))

(defn- landing [chunks tip fluid]
  (when-let [c (fillable-cauldron chunks tip fluid)]
    {:tip tip :cauldron c
     :delay (+ 50 (- (long (tip 1)) (long (c 1))))}))

(defn- transferred [chunks dim p st roll]
  (when-let [{:keys [pos state fluid]} (fluid-above chunks dim p st)]
    (let [prob (case fluid
                 :water water-chance
                 :lava lava-chance
                 nil)]
      (when (and prob (< (double roll) (double prob)))
        (when-let [tip (find-tip chunks p st 11)]
          (if (and (= :mud (block/block-of (long state)))
                   (= :water fluid))
            {:tip tip :changes [[pos (block/state :clay)]]}
            (landing chunks tip fluid)))))))

(defn drip
  "Returns the drip of the dripstone at p this tick.
  Returns nil when nothing drips. The result holds the tip it
  falls from and either block changes or the cauldron it fills
  and the delay before it lands. dim names the dimension."
  [chunks p st roll dim]
  (when (= :pointed-dripstone (block/type-of st))
    (let [roll (double (roll :drip))]
      (when (and (< roll water-chance) (start-pos? chunks p st))
        (transferred chunks dim p st roll)))))

(defn- created [chunks q dir th self]
  (let [wet (if (water-at? chunks q) :true :false)]
    (block/state self {:vertical-direction dir :thickness th
                       :waterlogged wet})))

(defn- merged [chunks q ^long tst self]
  (let [[a b] (if (= :up (dir-of tst))
                [(dir/up q) q]
                [q (dir/down q)])]
    [[a (created chunks a :down :tip_merge self)]
     [b (created chunks b :up :tip_merge self)]]))

(defn- grown [chunks from dir self]
  (let [target (toward from dir) n (chunk/at-void chunks target)]
    (cond
      (unmerged-tip? n (dir/opposite dir) self)
      (merged chunks target n self)
      (or (zero? n) (= :water (block/block-of n)))
      [[target (created chunks target dir :tip self)]])))

(defn- can-tip-grow? [chunks q ^long tst]
  (let [dir (dir-of tst) n (chunk/at-void chunks (toward q dir))]
    (cond
      (neg? n) false
      (some? (fluid-of n)) false
      (zero? n) true
      :else (let [self (block/block-of tst)]
              (unmerged-tip? n (dir/opposite dir) self)))))

(defn- grows-up? [chunks q self]
  (and (valid-placement? chunks q :up self)
       (not (water-at? chunks (dir/down q)))))

(defn- stalagmite-below [chunks tip self]
  (loop [i 0 q (dir/down tip)]
    (when (< i 10)
      (let [n (chunk/at-void chunks q)]
        (cond
          (neg? n) nil
          (some? (fluid-of n)) nil
          (and (unmerged-tip? n :up self) (can-tip-grow? chunks q n))
          (grown chunks q :up self)
          (grows-up? chunks q self)
          (grown chunks (dir/down q) :up self)
          (not (drips-through? n)) nil
          :else (recur (inc i) (dir/down q)))))))

(defn- can-grow? [chunks p self]
  (let [above (chunk/at-void chunks (dir/up p))]
    (and (= (grows-on self) (block/block-of above))
         (water-source-at? chunks (dir/up (dir/up p))))))

(defn- grow-tip [chunks tip self roll]
  (let [tst (chunk/at-void chunks tip)]
    (when (and (free-hanging? tst) (can-tip-grow? chunks tip tst))
      (if (< (double (roll :which)) 0.5)
        (grown chunks tip :down self)
        (stalagmite-below chunks tip self)))))

(defn- grow-changes [chunks p ^long st roll]
  (let [self (block/block-of st)
        len (get max-growth-length (block/type-of st) 7)]
    (when (can-grow? chunks p self)
      (when-let [tip (find-tip chunks p st len)]
        (grow-tip chunks tip self roll)))))

(defn random-changes [chunks p ^long st roll]
  (when (and (= :pointed-dripstone (block/type-of st))
             (< (double (roll :grow)) growth-chance)
             (start-pos? chunks p st))
    (grow-changes chunks p st roll)))

(defn- fall-changes [chunks p]
  (loop [q p acc []]
    (let [st (chunk/at-void chunks q)]
      (if-not (stalactite? st)
        acc
        (let [acc (conj acc [q (block/emptied st) [[:fall st]]])]
          (if (tip? st true) acc (recur (dir/down q) acc)))))))

(defn- wake
  "SpeleothemBlock.updateShape: a tick when the block at the base
  side changes and no longer holds it."
  [chunks _dim tick p _old side]
  (let [st (chunk/at-void chunks p)
        down? (stalactite? st)]
    (when (and (= side (if down? :up :down))
               (not (supported? chunks p st)))
      (+ (long tick) (if down? 2 1)))))

(defn- fallen [chunks p _ctx]
  (let [st (chunk/at-void chunks p)]
    (if (and (stalagmite? st) (not (supported? chunks p st)))
      [(block/destroyed p st)]
      (seq (fall-changes chunks p)))))

(def rule
  {:name   :dripstone
   :match? (fn [_chunks st _p] (speleothem? st))
   :wake   wake
   :due    fallen})

(defn- cauldron-fill [chunks p ctx]
  (when-let [tip (tip-above-cauldron chunks p)]
    (when-let [fluid (fill-fluid chunks (:dim ctx) tip)]
      (let [st (chunk/at-void chunks p)]
        (when (receives? st fluid)
          (when-let [st' (filled st fluid)]
            [[p st' [[:drip fluid]]]]))))))

(def cauldron-rule
  {:name   :cauldron-drip
   :match? (fn [_chunks st _p]
             (contains? cauldrons (block/type-of st)))
   :wake   (fn [_chunks _dim _tick _p _old _side] nil)
   :due    cauldron-fill})
