...
(defn- sugar-cane-supported? [chunks [x y z] st below]
  (or (= (block/block-of below) (block/block-of st))
      (and (block/tagged? below "supports_sugar_cane")
           (boolean (some (fn [[dx _ dz]]
                            (let [n (state-at chunks [(+ (long x) (long dx)) (dec (long y)) (+ (long z) (long dz))])]
...
(def ^:private supports
  (into {}
        (for [[classes f] [[[:torch :redstone-torch :candle] (fn [_c _p _st below _a] (holds-center-below? below))]
                           [[:wall-torch :redstone-wall-torch :ladder]
                            (fn [c p st _b _a] (attached-to? c p (dir/opposite (block/facing-of st))))]
                           [[:standing-sign :banner] (fn [_c _p _st below _a] (block/blocks-motion? (max 0 below)))]
                           [[:wall-banner :wall-sign] (fn [c p st _b _a] (wall-attached? c p st))]
                           [[:ceiling-hanging-sign] (fn [_c _p _st _b above] (holds-center-above? above))]
                           [[:wall-hanging-sign] (fn [c p st _b _a] (hanging-sign-held? c p st))]
                           [[:kelp :kelp-plant] (fn [_c _p _st below _a] (kelp-supported? below))]
                           [[:tall-seagrass] (fn [_c _p st below _a] (tall-seagrass-supported? st below))]
                           [[:azalea :wither-rose :nether-sprouts :nether-fungus :nether-roots]
                            (fn [_c _p st below _a] (vegetation-supported? (block/type-of st) st below))]
                           [[:mangrove-propagule] (fn [_c _p st below above] (propagule-supported? st below above))]
                           [[:chorus-flower :chorus-plant] (fn [c p st _b _a] (chorus/supported? c p st))]
                           [[:fire :soul-fire] (fn [c p st below _a] (fire-supported? c p st below))]
                           [[:mushroom] (fn [c p _st below _a] (mushroom-supported? c p below))]
                           [[:sugar-cane] (fn [c p st below _a] (sugar-cane-supported? c p st below))]
                           [[:cactus] (fn [c p st below _a] (cactus-supported? c p st below))]
                           [[:lily-pad] (fn [c p _st below _a] (lily-pad-supported? c p below))]
                           [[:snow-layer] (fn [_c _p _st below _a] (snow-supported? below))]
                           [[:wool-carpet :carpet] (fn [_c _p _st below _a] (not (zero? below)))]
                           [[:leaf-litter :coral-plant :coral-fan :base-coral-plant :base-coral-fan]
                            (fn [_c _p _st below _a] (and (not (neg? below)) (block/face-sturdy? below :up)))]
                           [[:lantern :weathering-lantern] (fn [_c _p st below above] (lantern-supported? st below above))]
                           [[:bell] (fn [c p st below above] (bell-supported? c p st below above))]
                           [[:cake :candle-cake] (fn [_c _p _st below _a] (block/legacy-solid? (max 0 below)))]
                           [[:grindstone] (fn [_c _p _st _b _a] true)]
                           [[:button :lever] (fn [c p st _b _a] (attachable? c p (dir/opposite (connected-direction st))))]
                           ...
                           [[:amethyst-cluster] (fn [c p st _b _a] (cluster-supported? c p st))]
                           [[:sea-pickle] (fn [_c _p _st below _a] (or (neg? below) (block/face-sturdy? below :up) (block/full-cube? below)))]
                           [[:cocoa] (fn [c p st _b _a] (cocoa-supported? c p st))]
                           [[:spore-blossom] (fn [c p _st _b above] (and (holds-center-above? above) (not (water? (state-at c p)))))]
                           [[:coral-wall-fan :base-coral-wall-fan]
                            (fn [c p st _b _a] (attached-to? c p (dir/opposite (block/facing-of st))))]
                           [[:vine] (fn [c p st _b _a] (pos? (vine-updated c p st)))]
                           [[:glow-lichen :multiface :sculk-vein] (fn [c p st _b _a] (pos? (multiface-updated c p st)))]
                           [[:scaffolding] (fn [c p _st _b _a] (< (scaffold-distance c p) 7))]
                           [[:mossy-carpet] (fn [c p st _b _a] (moss/carpet-supported? c p st))]
                           [[:hanging-moss] (fn [c p st _b _a] (moss/hanging-supported? c p st))]
                           [[:pointed-dripstone :sulfur-spike] (fn [c p st _b _a] (dripstone/supported? c p st))]
                           [[:big-dripleaf :big-dripleaf-stem :small-dripleaf] (fn [c p st _b _a] (dripleaf/supported? c p st))]
                           [[:hanging-roots] (fn [_c _p _st _b above] (and (not (neg? above)) (block/face-sturdy? above :down)))]
                           [[:farmland] (fn [_c _p _st _b above] (farmland-supported? above))]
                           [[:dirt-path] (fn [_c _p _st _b above] (dirt-path-supported? above))]
                           [[:crop :carrot :potato :beetroot :torchflower-crop]
                            (fn [c p st below _a] (crop-supported? c p st below))]
                           [[:pitcher-crop] (fn [c p st below _a] (pitcher-supported? c p st below))]
                           [[:rail :powered-rail :detector-rail]
                            (fn [_c _p _st below _a] (or (neg? below) (block/face-holds-rigid? below :up)))]
                           [[:pressure-plate :weighted-pressure-plate]
                            (fn [_c _p _st below _a] (or (neg? below) (block/face-holds-rigid? below :up) (block/face-holds-center? below :up)))]
                           [[:redstone-wire]
                            (fn [_c _p _st below _a] (or (neg? below) (block/face-sturdy? below :up) (= :hopper (block/block-of below))))]]
              k classes]
          [k f])))
...
(defn- lantern-fitted [chunks pos st pitch]
  (let [self (block/block-of st) props (block/props-of st)
    ...
    (pick chunks pos (if (pos? (double pitch)) [standing hanging] [hanging standing]))))

(defn- bell-fitted [chunks pos st face yaw]
  (let [self (block/block-of st) props (block/props-of st)
        face (long face)
...
(defn vine-updated ^long [chunks pos ^long st]
  (let [props (block/props-of st)
        props' (reduce (fn [m dir]
                         (if (= :true (get m dir))
                           (assoc m dir (if (vine-face-held? chunks pos st dir) :true :false))
                           m))
...
(defn- multiface-fitted [chunks pos st yaw pitch]
  (let [cur (state-at chunks pos)
...
(defn- rod-fitted [chunks pos st face]
  (let [f (block/facing-of st)
...
(defn- standing-or-wall-fitted [chunks pos st {:keys [yaw pitch face replacing?]}]
  (let [order (placement-order yaw pitch face replacing?)
        ...
        wall-state (first (for [dir order :when (contains? dir/horizontal-offset dir)
                                :when (supported? chunks pos (on-wall dir))]
                            (on-wall dir)))]
    (standing-or-wall order (when (supported? chunks pos st) st) wall-state)))

(defn- skull-fitted [chunks pos st {:keys [yaw pitch face replacing?]}]
  (let [order (placement-order yaw pitch face replacing?)
        wall (block/wall-block (block/block-of st))
        wall-state (first (for [dir order :when (contains? dir/horizontal-offset dir)
                                :when (not (block/can-be-replaced? (max 0 (state-at chunks (mapv + pos (dir/horizontal-offset dir))))))]
                            (block/state wall {:facing (dir/opposite dir)})))]
    (standing-or-wall order st wall-state)))

(defn- growing-plant-fitted [chunks pos st tick]
  (let [{:keys [head body dir]} (block/growing-plant (block/type-of st))
...
(defn- hanging-sign-fitted [chunks pos st yaw pitch sneaking?]
  (let [wall (block/wall-block (block/block-of st))
        ...
        wall-state (first (for [dir (look-order yaw pitch) :when (contains? dir/horizontal-offset dir)
                                :when (hanging-sign-held? chunks pos (on-wall dir))]
                            (on-wall dir)))
        ceiling (ceiling-sign chunks pos st yaw sneaking?)]
    (first (for [dir (look-order yaw pitch) :when (not= :down dir)
                 :let [cand (if (= :up dir) (when (supported? chunks pos ceiling) ceiling) wall-state)]
                 :when cand]
             cand))))
...
(defn- face-attached-fitted [chunks pos st face yaw pitch replacing?]
  (let [self (block/block-of st) props (block/props-of st)
...
(def ^:private fits
  (into {}
        (for [[classes f] [[[:button :lever :grindstone]
                            (fn [c p st {:keys [face yaw pitch replacing?]}] (face-attached-fitted c p st face yaw pitch replacing?))]
                           ...
                           [[:ceiling-hanging-sign]
                            (fn [c p st {:keys [yaw pitch sneaking?]}] (hanging-sign-fitted c p st yaw pitch sneaking?))]
                           [[:lantern :weathering-lantern] (fn [c p st {:keys [pitch]}] (lantern-fitted c p st pitch))]
                           [[:bell] (fn [c p st {:keys [face yaw]}] (bell-fitted c p st face yaw))]
                           [[:cocoa] (fn [c p st {:keys [yaw]}] (cocoa-fitted c p st yaw))]
                           [[:bamboo-stalk] (fn [c p _st _o] (bamboo-fitted c p))]
                           [[:weeping-vines :weeping-vines-plant :twisting-vines :twisting-vines-plant :cave-vines :cave-vines-plant]
                            (fn [c p st {:keys [tick]}] (growing-plant-fitted c p st tick))]
                           [[:end-rod] (fn [c p st {:keys [face]}] (rod-fitted c p st face))]
                           [[:mossy-carpet] (fn [c p st _o] (carpet-fitted c p st))]
                           [[:pointed-dripstone :sulfur-spike]
                            (fn [c p st {:keys [pitch sneaking?]}] (dripstone/placed c p st pitch sneaking?))]
                           [[:big-dripleaf] (fn [c p st _o] (dripleaf/leaf-placed c p st))]
                           [[:vine] (fn [c p st {:keys [yaw pitch]}] (vine-fitted c p st yaw pitch))]
                           [[:glow-lichen :multiface :sculk-vein] (fn [c p st {:keys [yaw pitch]}] (multiface-fitted c p st yaw pitch))]
                           [[:scaffolding] (fn [c p st _o] (when (< (scaffold-distance c p) 7) (scaffold-state c p st)))]
                           [[:campfire] (fn [c p st {:keys [yaw]}] (campfire/placed c p st yaw))]
                           [[:huge-mushroom] (fn [c p st _o] (mushroom/placed c p st))]
                           [[:farmland :dirt-path] (fn [c p st _o] (if (supported? c p st) st (gone-state st)))]]
              k classes]
          [k f])))
