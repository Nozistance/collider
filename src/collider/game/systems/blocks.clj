  ...
  (and (not use-item?)
       ...
       (let [cur (block-at world pos)]
         (or (contains? #{:flower-pot :candle :candle-cake :cake :composter :cave-vines :cave-vines-plant
             ...
             (poses? cur item)
             (contains? block/cauldron-types (block/type-of cur))
             (some? (sign/kind cur))
...
(defn- use-deltas [world eid pos face item cursor]
  (let [cur (block-at world pos) t (block/type-of cur)]
    (cond
      ...
      (contains? block/cauldron-types t) (cauldron-deltas world pos item (held-stack world eid))
    ...
    (when (or (= 0 status) (= 2 status))
      (if (pos? old)
        (into (vec (concat (jukebox-break-deltas world pos)
              ...
              (concat (change-deltas world [[pos (block/emptied old)]])
